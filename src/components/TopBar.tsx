...
import { LayoutTemplate, Undo2, Redo2, Moon, Sun, MoreHorizontal, CheckCircle2 } from 'lucide-react';
 ...
 */
export default function TopBar({ undo, redo, canUndo, canRedo, activeTab, setActiveTab, isDarkMode, setIsDarkMode }: TopBarProps) {
  return (
    <header className="min-h-14 border-b border-gray-800 bg-gray-950 flex flex-wrap items-center justify-between px-4 py-2 shrink-0 gap-y-2">
      ...
      <nav className="flex items-center gap-1 overflow-x-auto w-full md:w-auto order-3 md:order-2 pb-1 md:pb-0 scrollbar-hide">
        {['Project', 'Template', 'Design', 'Layout', 'Preview', 'Export'].map((item) => (
          <button
            key={item}
